                    ...
                    <div className="mt-4 sm:mt-6">
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                        {order.items.map((item, index) => (
                          <div key={index} className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-3 border-b border-gray-200 last:border-b-0 gap-3">
                            <div className="flex items-center space-x-3 sm:space-x-4">
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="font-medium text-gray-900 text-sm sm:text-base truncate">{item.product?.name || 'Unknown Product'}</div>
                                <div className="text-xs sm:text-sm text-gray-500">{item.product?.brand || 'N/A'}</div>
                                <div className="text-xs sm:text-sm text-gray-500">Quantity: {item.quantity}</div>
                              </div>
                            </div>
                            <div className="flex flex-col sm:text-right">
                              <div className="font-medium text-gray-900 text-sm sm:text-base">${item.price}</div>
                              {order.status === 'delivered' && (
                                <button
                                  onClick={() => openReviewModal(item.product, order._id)}
                                  className="mt-2 flex items-center justify-center sm:justify-start space-x-1 px-3 py-1 bg-blue-600 text-white text-xs sm:text-sm rounded-md hover:bg-blue-700 transition-colors w-fit"
                                >
